...
- **Keep the docs alive.** Any change to the public API must update the guides under `docs/`, the relevant `docs/api/*` reference page, the machine-readable [`docs/llms.txt`](docs/llms.txt), and the compile-checked `examples/basic-usage.ts` in the **same** change. The MkDocs site (published to <https://docs.yasuo.gg/>) is the canonical reference; `bun run docs:build` must stay green (`--strict`). Code fences require `pymdown-extensions>=10.14` (pinned in `docs/requirements.txt`) — older releases silently drop syntax highlighting.
- **Keep `docs/llms.txt` in sync (the LLM-facing manual).** `docs/llms.txt` is the single, self-contained reference an LLM/coding agent consumes to use yasuo; MkDocs copies it verbatim to <https://docs.yasuo.gg/llms.txt> at build time (it's a non-`.md` static file, so it needs no `nav`/`mkdocs.yml` entry and doesn't trip `--strict`). It is **hand-maintained, not generated** — so every change to the public surface (a namespace method's name/params/return, the `execute()`/config/error/enum/cache/rate-limit/middleware/pagination model, or a new export from `src/index.ts`) must update `docs/llms.txt` in the **same** change, alongside the guides. Match the wording to reality; if `src/` and `docs/llms.txt` disagree, `src/` wins — fix the file. Don't pin the package version inside it (that's `package.json`'s job), and keep it terse: it mirrors the docs for machines, it is not a second copy of every guide.
...
**Before "done":** `bun run typecheck`, `bun run lint`, and `bun run test:unit` (coverage-gated) must all be green; new logic needs a unit test; new public surface is exported from `src/index.ts`; docs, `examples/basic-usage.ts`, and `docs/llms.txt` updated to match; and `package.json`'s `version` is bumped ahead of the remote (patch, unless it was already bumped).
